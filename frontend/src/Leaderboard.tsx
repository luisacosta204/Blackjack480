...
import { getLeaderboard, LBRange } from './api/leaderboard';

export default function Leaderboard() {
  const [rows, setRows] = useState<any[]>([]);
  const [range, setRange] = useState<LBRange>('all');

  useEffect(() => { getLeaderboard(range).then(setRows); }, [range]);
    ...
    <div style={s.shell}>
      ...
      <div style={{display:'flex', gap:8, marginBottom:12}}>
        {(['all','week','day'] as LBRange[]).map(r =>
          <button key={r} style={range===r? s.btnPrimary : s.btn} onClick={()=>setRange(r)}>{r}</button>
        )}
      </div>
      <table style={s.table}>
        <thead><tr><th>User</th><th>Wins</th><th>Losses</th><th>Score</th><th>Last Played</th></tr></thead>
        <tbody>
          {rows.map((r,i)=>(
            <tr key={i}>
              <td>{r.username}</td><td>{r.wins}</td><td>{r.losses}</td><td>{r.score}</td><td>{new Date(r.last_played).toLocaleString()}</td>
            </tr>
          ))}
        </tbody>
      </table>
