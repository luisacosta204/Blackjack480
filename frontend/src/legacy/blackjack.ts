...
export function initBlackjack(rootEl: HTMLElement) {
    ...
    if (anyPlayerBJ) {
      for (const hand of hands) {
        if (isBlackjack(hand.cards)) {
          ...
          const payout = Math.floor(stake * 3 / 2) + stake; // 3:2 + return stake
          bank += payout;
          saveBank();
          hand.finished = true;
        }
      }
      if (hands.every((h) => h.finished)) endRoundAndReport();
    }
    renderTable(true);
  }
  ...
  function onDouble() {
    ...
    bank -= hand.bet; // add second stake
    saveBank();
    updateBankBadge();
    hand.doubled = true;
    hand.cards.push(drawCard());
  ...
  function settleAll(naturalCheck: boolean) {
    ...
    if (dealerBJ) {
      // pay insurance 2:1 (player already paid the insurance stake)
      for (const hand of hands) {
        if (hand.insurance > 0) {
          bank += hand.insurance * 3; // return stake + 2x
          hand.insurance = 0;
        }
      }
      saveBank();
    ...
    for (const hand of hands) {
      ...
      if (naturalCheck && dealerBJ) {
        // natural vs natural -> push (return stake)
        if (isBlackjack(hand.cards)) payout = stake;
      } else {
        if (pv > 21) {
          payout = 0; // loss (stake already deducted at deal/double/split time)
        } else if (dealerVal > 21 || pv > dealerVal) {
          payout = stake * 2; // return stake + win
