...
type Props = {
  onBack?: () => void; // optional: let App.tsx wire a back action
};

export default function Blackjack({ onBack }: Props) {
  const rootRef = useRef<HTMLDivElement>(null);

  // Inline script from your HTML (avatar/username header logic) + dynamic game script loader
  useEffect(() => {
    // --- Header user info init (from your <script> in blackjack.html) ---
    const avatar = document.getElementById('headerAvatar') as HTMLImageElement | null;
    const usernameEl = document.getElementById('headerUsername') as HTMLElement | null;
    ...
    if (storedAvatar && avatar) avatar.src = storedAvatar;

    const storageHandler = (e: StorageEvent) => {
      if (e.key === 'bj21.avatar' && e.newValue && avatar) {
        avatar.src = e.newValue;
      }
    };
    window.addEventListener('storage', storageHandler);

    // --- Load your legacy game script from /public (keeps your original IDs working) ---
    // Place blackjack.js at: frontend/public/blackjack/blackjack.js
    const script = document.createElement('script');
    script.src = '/blackjack/blackjack.js?v=dev1';
    script.defer = true;
    document.body.appendChild(script);

    // Cleanup: remove listeners and script tag when unmounting
    return () => {
  ...
  // Optional "back" button handler (stays in React land if provided)
  function handleBackClick(e: React.MouseEvent) {
    e.preventDefault();
    if (onBack) onBack();
    // otherwise, do nothing (or navigate with your router)
  }
  ...
  return (
    <>
      ...
      {/* Main content */}
      <main className="container stack" ref={rootRef}>
